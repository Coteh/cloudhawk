        ...
        sampleQueries.forEach((sampleQuery, i) => {
            ...
            cy.get("div[role='tablist']").within(() => {
                cy.get(`button[data-index='${i}']`)
                    .should('contain.text', sampleQuery.queryId.slice(0,20) + "...")
                    .click();
                cy.root()
                    .parent()
    ...
    it('should allow user to select log groups to query from', () => {
        ...
        cy.intercept(/\/log_groups\?logGroupPrefix=.*$/, {
            ...
            body: {
                status: 'success',
                data: {
                    logGroups: new Array(10).fill(0).map((_, i) => `/aws/lambda/test-${i}`),
                },
            },
        }).as('logGroupQuery');
        ...
        cy.get("div[role='tabpanel']:not([hidden])").within(() => {
            ...
            cy.contains("Log Groups:")
                ...
                .within(() => {
                    for (let i = 2; i <= 4; i++) {
                        cy.contains(`/aws/lambda/test-${i}`).should("be.visible");
                    }
                });
            ...
            cy.wait("@query").then(interception => {
                for (let i = 0; i < 5; i++) {
                    cy.wrap(interception.request.body).should("include", `/aws/lambda/test-${i}`);
                }
            });
            ...
            cy.wait("@query").then(interception => {
                for (let i = 0; i < 5; i++) {
                    if (i === 3) {
                        cy.wrap(interception.request.body).should("not.include", `/aws/lambda/test-${i}`);
                    } else {
                        cy.wrap(interception.request.body).should("include", `/aws/lambda/test-${i}`);
                    }
                }
            ...
            // TODO: Clicking anywhere besides the prefix input or the log groups dialog should close the log groups dialog
            // cy.get("input[name='log-group-prefix']")
            //     .should('be.visible')
            //     .siblings("div")
            //     .should("be.visible")
